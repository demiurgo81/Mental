...
# Función para guardar expresión regular en MongoDB
def guardar_expresion_regular(titulo, regex):
    try:
        collection.insert_one({"titulo": titulo, "regex": regex, "timestamp": time.time()})
...
class Aplicacion(tk.Tk):
    def __init__(self):
        ...
        # Campo para ingresar título y expresión regular
        ...
        # Botón para guardar expresión regular
        self.btn_guardar = tk.Button(self, text="Guardar Expresión Regular", command=self.guardar_expresion)
        ...
        self.expresiones_var = tk.StringVar(self)
        self.menu_expresiones = tk.OptionMenu(self, self.expresiones_var, ())
        self.menu_expresiones.pack(pady=5)
        ...
        self.archivos = []
        self.cargar_expresiones_guardadas()
    ...
    def guardar_expresion(self):
        ...
        if titulo and regex:
            guardar_expresion_regular(titulo, regex)
            self.cargar_expresiones_guardadas()
        else:
            messagebox.showwarning("Advertencia", "Debe ingresar un título y una expresión regular")

    def cargar_expresiones_guardadas(self):
        expresiones = obtener_expresiones_guardadas()
        self.menu_expresiones['menu'].delete(0, 'end')
        for expresion in expresiones:
            self.menu_expresiones['menu'].add_command(
                label=expresion['titulo'],
                command=lambda e=expresion: self.seleccionar_expresion(e)
            )
        self.expresiones_var.set("")

    def seleccionar_expresion(self, expresion):
        self.entry_regex.delete(0, 'end')
        self.entry_regex.insert(0, expresion['regex'])
    ...
    def extraer_coincidencias(self):
        ...
        regex = self.entry_regex.get()
        if not regex:
            messagebox.showwarning("Advertencia", "Debe ingresar o seleccionar una expresión regular")
            return
